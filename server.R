server <- function(input, output, session) {
  ...
  showNotification("Created by Elvriana Elvani", duration = NULL, type = "message")
...
# Total Flights (server) ------------------------------------------
  output$total_flights <- renderValueBox({
    ...
    base_flights() %>%
      ...
      as.integer() %>%
      prettyNum(big.mark = ",") %>%
      valueBox(icon = icon("chart-bar"), color = "aqua",subtitle = "Number of Flights")
  })
...
# Avg per Day (server) --------------------------------------------
  output$per_day <- renderValueBox({
    ...
    base_flights() %>%
      ...
      round() %>%
      prettyNum(big.mark = ",") %>%
      valueBox(icon = icon("balance-scale"), color = "olive",subtitle = "Average Flights per day") 
  })
